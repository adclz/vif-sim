...
class NodeDispatcher {
    ...
    constructor(server_id) {
        ...
        this.dispatcher_worker = new Worker(dispatcher_string.substring(
            dispatcher_string.indexOf("{") + 1,
            dispatcher_string.lastIndexOf("}")
        ), {eval: true})
        this.dispatcher_worker.on("error", error => console.error(error))
        this.dispatcher_worker.postMessage({type: 0, server_id})
        console.log(`Node dispatcher is launched with id ${this.server_id}`)
    }

    addPlugin(interval, name) {
        if (this.plugins.includes(name)) {
            console.log(`Could not save plugin ${name}: A plugin already exists with this name`)
    ...
    parentPort.on("message", async (e) => {
        switch (e.type) {
            ...
            case 1: {
                ...
                const plugin_worker = new Worker(plugin_string.substring(
                    plugin_string.indexOf("{") + 1,
                    plugin_string.lastIndexOf("}")
                ), {eval: true})
                plugin_worker.on("error", error => console.error(error))
                plugin_worker.postMessage({type: 0, id: server_id, name: e.name, interval: e.interval,})
                plugins.push(plugin_worker)
                console.log(`New plugin registered on dispatcher with name ${e.name} and interval ${e.interval}`)
                break;
            }

            // Store
            case 2: {
                plugins.forEach(plugin => plugin.postMessage({type: 2, store: e.store}))
            }
        }
    })
    ...
    const plugin = function () {
        ...
        parentPort.on("message", async (e) => {
            switch (e.type) {
                // Init
                case 0:
                    checkBroadcast(e.id, e.name, e.interval)
                    break;
                // Store
                case 2:
                    store = e.store
