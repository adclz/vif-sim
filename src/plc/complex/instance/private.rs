                ...
                Section::Input => {
                    members
                        ...
                        .try_for_each(|(target_path, source)| {
                            let target = self.get_interface().try_get_nested(&target_path)
                                .ok_or(error!(format!("Could not find a valid reference in instance interface for path {:?}, Current interface: {}", &target_path, self.get_interface())))?;
                ...
                Section::InOut => {
                    members
                        ...
                        .try_for_each(|(target_path, source)| {
                            ...
                            self.get_mut_interface().try_replace_pointer_nested(target_path, &source);
                            Ok(())
                        })
                },
                _ => Ok(())
