...
impl BuildJsonOperation for TimerStateMachine {
    fn build(
        ...
        let preset_var = self
            ...
            .ok_or(error!(format!("Invalid preset var for timer {}", self.preset_var), format!("Build Timer -> preset var")))?;
        let timer_var = self
            .timer_var
            .solve_as_local_pointer(_interface, template, registry, channel)
            .ok_or(error!(format!("Invalid timer var for timer {}", self.preset_var), format!("Build Timer -> timer var")))?;
