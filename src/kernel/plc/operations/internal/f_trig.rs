...
impl FileTraceBuilder for F_Trig {
    fn get_trace(&self) -> &Option<FileTrace> {
        &self.trace
    }
}

impl NewJsonOperation for F_Trig {
        ...
        Self: Clone,
    {
        ...
        let input = parse_json_target(input).map_err(|e| {
            e.add_sim_trace("Build #F_Trig -> input")
...
impl BuildJsonOperation for F_Trig {
    fn build(
        &self,
        ...
        Ok(Box::new(Operation::new(
            MaybeHeapOrStatic(Some(HeapOrStatic::Static(&"Falling Edge"))),
            move |channel| {
