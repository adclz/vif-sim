...
export class Container extends VifEventEmitter<Hooks> {
    ...
    constructor() {
        ...
        this.worker.addEventListener("message", (ev: MessageEvent<FromCommand>) => {
            switch (ev.data.command) {
                case From.ContainerReady:
                    this.container_id = ev.data.id
                    this.emit("container:ready", ev.data.id)
                    break;
                case From.LoadPlugin:
                    this.emit("plugin:loaded", ev.data.status)
                    break;
                case From.LoadProviderStatus:
                    this.emit("parse:provider", ev.data.status)
                    break;
                case From.LoadProgramStatus:
                    this.emit("parse:program", ev.data.status)
                    break;
                case From.RuntimeCommandsInt32:
                    if (ev.data.sab)
                        this.command_store = new CommandStore(new Int32Array(ev.data.sab), this.command_lock_int32)
                    break;
            }
        })
    }
